-- Fix get_admin_broadcast_history and get_user_notifications to include is_rtl column
-- IMPORTANT: Must DROP functions first because return type changed (PostgreSQL cannot use CREATE OR REPLACE)

-- Drop ALL versions of get_admin_broadcast_history (return type changed)
DROP FUNCTION IF EXISTS public.get_admin_broadcast_history();

-- Recreate get_admin_broadcast_history with is_rtl column
...
BEGIN
    ...
    GROUP BY an.id, an.title, an.message, an.priority, an.deep_link, an.action_button_text,
             an.sent_by, p.email, an.sent_at, an.expires_at, an.auto_dismiss_hours, an.is_rtl
    ORDER BY an.sent_at DESC;
END;
$$;

-- Drop ALL versions of get_user_notifications (return type changed)
-- Must drop both the no-parameter version AND the parameterized version from earlier migrations
DROP FUNCTION IF EXISTS public.get_user_notifications();
DROP FUNCTION IF EXISTS public.get_user_notifications(boolean);

-- Recreate get_user_notifications with is_rtl column (no parameters - simplified API)
